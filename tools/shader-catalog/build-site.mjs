...
.stage .cap{position:absolute;left:14px;right:14px;bottom:14px;display:flex;justify-content:space-between;align-items:center;gap:10px;
  padding:10px 14px;border-radius:14px;background:rgba(4,16,11,.72);-webkit-backdrop-filter:blur(10px);backdrop-filter:blur(10px);font-family:var(--mono);font-size:12px;color:var(--mute);pointer-events:none}
...
.toolbar{position:sticky;top:0;z-index:5;background:rgba(4,16,11,.85);-webkit-backdrop-filter:blur(12px);backdrop-filter:blur(12px);border-bottom:1px solid var(--line);padding:12px 0}
...
.badge{position:absolute;top:10px;left:10px;padding:3px 8px;border-radius:999px;background:rgba(4,16,11,.7);-webkit-backdrop-filter:blur(6px);backdrop-filter:blur(6px);font-family:var(--mono);font-size:10.5px;color:var(--ink)}
.empty{color:var(--mute);padding:40px 0;text-align:center}

/* modal */
.modal{position:fixed;inset:0;z-index:20;display:none;align-items:center;justify-content:center;padding:20px;background:rgba(0,0,0,.72);-webkit-backdrop-filter:blur(6px);backdrop-filter:blur(6px)}
.modal.open{display:flex}
.sheet{position:relative;width:min(1140px,100%);max-height:100%;overflow:auto;overscroll-behavior:contain;background:var(--surface);border:1px solid var(--line);border-radius:24px;
  display:grid;grid-template-columns:minmax(300px,520px) 1fr;gap:0}
/* Single column only when the screen is tall enough to scroll the stack —
   landscape phones (short viewports) keep two compact columns instead. */
@media (max-width:900px) and (min-height:521px){.sheet{grid-template-columns:1fr}}
@media (max-width:900px) and (max-height:520px){.sheet{grid-template-columns:minmax(240px,320px) 1fr}}
/* min-width:0 lets the panes shrink below the code block's longest line;
   without it pre.code's min-content forces the column wider than the sheet. */
.pane{padding:22px;min-width:0}
.pane.left{border-right:1px solid var(--line);background:var(--low)}
@media (max-width:900px) and (min-height:521px){.pane.left{border-right:0;border-bottom:1px solid var(--line)}}
@media (max-height:520px){.view{width:min(100%,calc(100vh - 150px));margin-inline:auto}}
...
@media (max-width:900px){
  .chips{flex-wrap:nowrap;overflow-x:auto;overscroll-behavior-x:contain;scrollbar-width:none;padding-bottom:4px}
  .chips::-webkit-scrollbar{display:none}
  .chip{flex:0 0 auto}
  /* Pin the close button to the viewport so it stays reachable while the
     sheet scrolls (on narrow screens the sheet is nearly full-width). */
  .close{position:fixed;top:14px;right:14px}
}
@media (max-height:520px){.modal{padding:8px}}
@media (max-width:700px){
  .wrap{padding:0 14px}
  header.top{flex-wrap:wrap;padding:12px 0;row-gap:10px}
  nav.links{flex-wrap:nowrap;overflow-x:auto;max-width:100%;scrollbar-width:none}
  nav.links::-webkit-scrollbar{display:none}
  nav.links a{flex:0 0 auto;padding:6px 11px}
  .hero{padding:26px 0 18px;gap:18px}
  .hero p.lead{font-size:15.5px}
  .stats{gap:14px;margin:14px 0 18px}
  ...
  .pane{padding:14px}
  .pane.right h2{font-size:21px}
  pre.code{max-height:42vh;font-size:12px}
  .rowbtns .mini{flex:1 1 auto;text-align:center}
}
@media (pointer:coarse){
  .card,.chip,.mini,.btn,.tab,.close,nav.links a{touch-action:manipulation}
  .ctl input[type=range]{height:30px}
  .ctl input[type=color]{width:52px;height:36px}
