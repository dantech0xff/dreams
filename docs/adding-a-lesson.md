...
6. Rerun the section 6 commands; `lessons.json` and the README table pick the category up from the enum. The "N lessons in M categories" line inside README's catalog block is generated too, but update the hand-written counts elsewhere in `README.md` (the `65` lesson total) and in `docs/learning-path.md` (`10 category cards`, per-category counts).
